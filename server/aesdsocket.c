...
void managesocket(int socket_t) {
  ...
  int log_fd = open(LOGFILE_PATH, O_CREAT | O_RDWR | O_TRUNC, 0666);
  if (log_fd == -1) {
  ...
  while (graceful_exit_handler == false) {
    ...
    int recv_idx = 0;
    //Malloc space of receiving data
    recv_data = malloc(sizeof(char)*BUFFER_SIZE*receive_blocks);
    if(!recv_data){
...
//          int bytes_read_count;
          ...
          int bytes_sent_count = 0;
          int bytes_read_count = read(log_fd, log_read, BUFFER_SIZE);
          if(bytes_read_count != 0){
          	bytes_sent_count = bytes_read_count;
          }
          ...
          if(bytes_read_count == -1){
          	...
          	syslog(LOG_ERR, "Error occured while reading from file = %s. Exiting...", strerror(errno));
          	exit(-1);
          }
          ...
          int bytes_sent = -1;
          int bytes_sent_offset = 0;
          
          while(bytes_sent != 0){
          	bytes_sent = send(conn_fd, &log_read[bytes_sent_offset], bytes_sent_count, 0);
          	if(bytes_sent == -1){
          		if(errno == EINTR)
          			continue;
...
int main(int argc, char ** argv) {
  ...
  hints.ai_family = AI_PASSIVE; //chose any protocol as system sees fit
